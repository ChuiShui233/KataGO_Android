...
// File-scope recovery context - set by each tune* function before testAllConfigs,
// used inside testAllConfigs to record blacklist/checkpoint/success/failure.
static OpenCLTuningRecovery::State* s_recoveryState = nullptr;
static std::string s_recoveryKernelName;
...
std::string OpenCLTuneParams::saveToString(const OpenCLTuneParams& config) {
  ostringstream out;
  out << TUNEPARAMS_VERSION_LINE << "\n";
  out << "#canUseFP16Storage" << "\n";
  out << config.canUseFP16Storage << "\n";
...
static bool testAllConfigs(
  ...
  for(int i = 0; i<configs.size(); i++) {
    string descStr = getDesc(configs[i]);

    // Recovery: blacklist check - skip configs that previously crashed/failed
    if(s_recoveryState != nullptr && !s_recoveryKernelName.empty()) {
      if(s_recoveryState->skipIfBlacklisted(s_recoveryKernelName, descStr)) {
        if(verboseTuner) {
          out << "Tuning " << i << "/" << configs.size() << " skipped (blacklisted): " << descStr << endl;
        }
        continue;
      }
    }

    // Recovery: record that we're about to benchmark this config
    if(s_recoveryState != nullptr && !s_recoveryKernelName.empty()) {
      s_recoveryState->recordCaseAttempt(s_recoveryKernelName, descStr, (long long)i);
    }
    ...
    numTested++;
    if(accums.bad) {
      // Recovery: record failure so it gets blacklisted
      if(s_recoveryState != nullptr && !s_recoveryKernelName.empty()) {
        s_recoveryState->recordCaseFailure(s_recoveryKernelName, descStr, (long long)i, getErrorMessage(accums.badErr));
      }
      if(verboseErrors) {
      ...
      // Recovery: record success for checkpoint
      if(s_recoveryState != nullptr && !s_recoveryKernelName.empty()) {
        s_recoveryState->recordCaseSuccess(s_recoveryKernelName, descStr, (long long)i, "");
      }
...
static void tuneXGemmDirect(
  ...
  s_recoveryKernelName = "xGemmDirect";
  testAllConfigs(
...
static bool tuneHGemmWmma(
  ...
  s_recoveryKernelName = "hGemmWmma";
  bool suc = testAllConfigs(
    stopOnReferenceImplFail,
    configs,
    currentConfig,
...
static bool tuneHGemmWmmaNCHW(
  ...
  s_recoveryKernelName = "hGemmWmmaNCHW";
  bool suc = testAllConfigs(
    stopOnReferenceImplFail,
    configs,
    currentConfig,
...
static void tuneTransform(
  ...
  double bestKernelsPerSecond = 0.0;
  double errorToleranceScale = 0.005;
  s_recoveryKernelName = "conv3x3_transform";
...
static void tuneUntransform(
  ...
  double bestKernelsPerSecond = 0.0;
  double errorToleranceScale = 0.005;
  s_recoveryKernelName = "conv3x3_untransform";
...
static void tuneTransformerAttention(
  ...
  double bestKernelsPerSecond = 0.0;
  double errorToleranceScale = 0.005;
  s_recoveryKernelName = "transformer_attention";
...
static void tunePointWise(
  ...
  s_recoveryKernelName = "pointWise";
...
static void tuneAddChannelBiasesNCHW(
  ...
  double bestKernelsPerSecond = 0.0;
  double errorToleranceScale = 0.005;
  s_recoveryKernelName = "addChannelBiasesNCHW";
...
static void tuneTransformerRMSNorm(
  ...
  double bestKernelsPerSecond = 0.0;
  double errorToleranceScale = 0.05;
  s_recoveryKernelName = "transformerRMSNorm";
...
static void tuneSpatialRMSNorm(
  ...
  double bestKernelsPerSecond = 0.0;
  double errorToleranceScale = 0.05;
  s_recoveryKernelName = "spatialRMSNorm";
...
void OpenCLTuner::tune(
  ...
  bool verboseTuner,
  OpenCLTuneParams& tunedConfig,
  OpenCLTuningRecovery::State* recoveryState
) {
  // Activate incremental autotuning recovery if provided
  if(recoveryState != nullptr) {
    recoveryState->beginTuning();
    s_recoveryState = recoveryState;
  }
  ...
  double bestXGemmDirectKernelsPerSecond = 0.0;
  {
    ...
    tuneXGemmDirect(
     ...
     if(recoveryState != nullptr)
       recoveryState->saveIncrementalConfig(OpenCLTuneParams::saveToString(currentConfig));
   }
  ...
  // Deactivate recovery state before finishing
  s_recoveryState = nullptr;
  s_recoveryKernelName.clear();

  // If recovery state was active, finalize with the tuned config text
  if(recoveryState != nullptr) {
    std::string configText = OpenCLTuneParams::saveToString(tunedConfig);
    recoveryState->finishTuning(configText);
  }
...
OpenCLTuneParams OpenCLTuner::loadOrAutoTune(
  ...
  // Clear any stale tuning markers from a previous run that crashed mid-tune.
  OpenCLTuningRecovery::recoveryRemoveFile(dir + "/tuning_copy.marker");
  OpenCLTuningRecovery::recoveryRemoveFile(dir + "/tuning_active.marker");
  ...
  // If no tune file for this exact model, try to copy from another model on the same GPU.
  // Tuning parameters are GPU-specific, not model-specific, so any existing tune file
  // for the same GPU and board size works well as a starting point.
  if(autoCopyTuneFiles) {
    string copyMarkerPath = dir + "/tuning_copy.marker";
    OpenCLTuningRecovery::recoveryWriteFileAtomic(copyMarkerPath, "1");
    string gpuNameForFile;
    ...
    string prefix = Global::strprintf("tune%d_gpu%s_x%d_y%d_", TUNER_VERSION, gpuNameForFile.c_str(), nnXLen, nnYLen);
    string prefixFull = Global::strprintf("tune%d_gpu%s_x%d_y%d_", TUNER_VERSION, gpuNameForFile.c_str(), (int)NNPos::MAX_BOARD_LEN, (int)NNPos::MAX_BOARD_LEN);
    try {
      vector<string> files = FileUtils::listFiles(dir);
      for(size_t i = 0; i<files.size(); i++) {
        if((files[i].compare(0, prefix.size(), prefix) == 0 || files[i].compare(0, prefixFull.size(), prefixFull) == 0)
           && files[i].find(".txt") != string::npos
           && files[i] != OpenCLTuner::defaultFileName(gpuName, nnXLen, nnYLen, modelInfo)) {
          string srcPath = dir + "/" + files[i];
          string dstPath = dir + "/" + OpenCLTuner::defaultFileName(gpuName, nnXLen, nnYLen, modelInfo);
          try {
            string content = FileUtils::readFile(srcPath);
            std::ofstream out(dstPath);
            if(out.good()) {
              out << content;
              out.close();
              if(logger != NULL)
                logger->write("[AutotuneRecovery] Copied tuning parameters from " + files[i] + " to " + OpenCLTuner::defaultFileName(gpuName, nnXLen, nnYLen, modelInfo));
              OpenCLTuneParams loadedParams = loadFromTunerFile(dstPath, logger);
              OpenCLTuningRecovery::recoveryRemoveFile(copyMarkerPath);
              return loadedParams;
            }
          }
          catch(...) {}
  ...
  //But first check if the Android layer has requested recovery from a previous crash
  {
    string recoveryCopyMarker = dir + "/tuning_copy.marker";
    OpenCLTuningRecovery::recoveryWriteFileAtomic(recoveryCopyMarker, "1");
    std::string recoveryMarker = OpenCLTuningRecovery::State::recoveryMarkerPath(dir);
    std::string safeDefaultsMarker = OpenCLTuningRecovery::State::safeDefaultsMarkerPath(dir);
    // Check for safe defaults marker first (highest priority)
    {
      std::ifstream marker(safeDefaultsMarker.c_str());
      if(marker.good()) {
        marker.close();
        if(logger != NULL)
          logger->write("[AutotuneRecovery] Safe defaults marker detected, attempting restore");
        // Try to restore from checkpoint
        OpenCLTuningRecovery::Profile prof;
        prof.kataGoVersion = Version::getKataGoVersion();
        prof.gpuName = gpuName;
        prof.modelVersion = modelInfo.modelVersion;
        prof.trunkNumChannels = modelInfo.trunkNumChannels;
        prof.midNumChannels = modelInfo.midNumChannels;
        prof.regularNumChannels = modelInfo.regularNumChannels;
        prof.gpoolNumChannels = modelInfo.gpoolNumChannels;
        prof.transformerHeadDim = modelInfo.transformerHeadDim;
        prof.transformerVHeadDim = modelInfo.transformerVHeadDim;
        prof.transformerNumHeads = modelInfo.transformerNumHeads;
        prof.transformerNumKVHeads = modelInfo.transformerNumKVHeads;
        prof.transformerFFNChannels = modelInfo.transformerFFNChannels;
        prof.nnXLen = nnXLen;
        prof.nnYLen = nnYLen;
        prof.batchSize = OpenCLTuner::DEFAULT_BATCH_SIZE;
        OpenCLTuningRecovery::State st(dir, prof, OpenCLTuner::defaultFileName(gpuName, nnXLen, nnYLen, modelInfo), "", NULL);
        if(st.isCheckpointForThisProfile() && st.hasLastSuccessful()) {
          if(st.restoreLastSuccessfulTuneFile()) {
            if(logger != NULL)
              logger->write("[AutotuneRecovery] Restored last successful tuning profile from checkpoint");
            st.clearRecoveryMarkers();
            OpenCLTuningRecovery::recoveryRemoveFile(recoveryCopyMarker);
            // Now try to load the restored tune file
            try {
              OpenCLTuneParams loadedParams = loadFromTunerFile(openCLTunerFile,logger);
              return loadedParams;
            }
              ...
              (void)e;
              if(logger != NULL)
                logger->write("[AutotuneRecovery] WARNING: restored tune file failed to load, proceeding with autotuning");
            }
          }
        }
        // If we get here, restoration failed - clear markers and fall through to autotuning
        st.clearRecoveryMarkers();
      }
    }
    // Check for recovery marker
    {
      std::ifstream marker(recoveryMarker.c_str());
      if(marker.good()) {
        marker.close();
        if(logger != NULL)
          logger->write("[AutotuneRecovery] Recovery marker detected, attempting restore from checkpoint");
        OpenCLTuningRecovery::Profile prof;
        prof.kataGoVersion = Version::getKataGoVersion();
        prof.gpuName = gpuName;
        prof.modelVersion = modelInfo.modelVersion;
        ...
        if(st.isCheckpointForThisProfile() && st.hasLastSuccessful()) {
          if(st.restoreLastSuccessfulTuneFile()) {
            if(logger != NULL)
              logger->write("[AutotuneRecovery] Restored last successful tuning profile");
            st.clearRecoveryMarkers();
            OpenCLTuningRecovery::recoveryRemoveFile(recoveryCopyMarker);
            try {
              OpenCLTuneParams loadedParams = loadFromTunerFile(openCLTunerFile,logger);
        ...
        // Restoration failed or no checkpoint - clear markers and fall through to fresh autotuning
        st.clearRecoveryMarkers();
        if(logger != NULL)
          logger->write("[AutotuneRecovery] No valid checkpoint found, proceeding with fresh autotuning");
      }
    }
  }

  // Even without a recovery marker, check if the checkpoint has a lastSuccessfulConfig
  // that we can restore directly. This handles the case where the process crashed
  // during autotuning but the Android layer didn't write a recovery marker.
  {
    OpenCLTuningRecovery::Profile prof;
    prof.kataGoVersion = Version::getKataGoVersion();
    prof.gpuName = gpuName;
    ...
    if(st.isCheckpointForThisProfile() && st.hasLastSuccessful()) {
      if(logger != NULL)
        logger->write("[AutotuneRecovery] Found checkpoint with lastSuccessfulConfig, restoring without marker");
      if(st.restoreLastSuccessfulTuneFile()) {
        if(logger != NULL)
          logger->write("[AutotuneRecovery] Restored last successful tuning profile from checkpoint");
        st.clearRecoveryMarkers();
  ...
  // Signal to the app layer that real autotuning is about to start. The loading
  // screen polls for this marker to show a tuning-in-progress message.
  OpenCLTuningRecovery::recoveryRemoveFile(dir + "/tuning_copy.marker");
  string autotuneMarkerPath = dir + "/tuning_active.marker";
  OpenCLTuningRecovery::recoveryWriteFileAtomic(autotuneMarkerPath, "1");
  ...
  // Set up incremental autotuning recovery
  // This creates a checkpoint after each successful benchmark case, so if the process
  // crashes during autotuning, the Android watchdog can restore the last successful config.
  OpenCLTuningRecovery::Profile recoveryProf;
  recoveryProf.kataGoVersion = Version::getKataGoVersion();
  recoveryProf.gpuName = gpuName;
  recoveryProf.modelVersion = modelInfo.modelVersion;
  recoveryProf.trunkNumChannels = modelInfo.trunkNumChannels;
  recoveryProf.midNumChannels = modelInfo.midNumChannels;
  recoveryProf.regularNumChannels = modelInfo.regularNumChannels;
  recoveryProf.gpoolNumChannels = modelInfo.gpoolNumChannels;
  recoveryProf.transformerHeadDim = modelInfo.transformerHeadDim;
  recoveryProf.transformerVHeadDim = modelInfo.transformerVHeadDim;
  recoveryProf.transformerNumHeads = modelInfo.transformerNumHeads;
  recoveryProf.transformerNumKVHeads = modelInfo.transformerNumKVHeads;
  recoveryProf.transformerFFNChannels = modelInfo.transformerFFNChannels;
  recoveryProf.nnXLen = nnXLen;
  recoveryProf.nnYLen = nnYLen;
  recoveryProf.batchSize = OpenCLTuner::DEFAULT_BATCH_SIZE;
  string tuneFileName = OpenCLTuner::defaultFileName(gpuName, nnXLen, nnYLen, modelInfo);

  // Log function that writes to the KataGo logger
  std::function<void(const std::string&)> recoveryLogFn;
  if(logger != NULL) {
    recoveryLogFn = [logger](const std::string& msg) {
      logger->write("[AutotuneRecovery] " + msg);
    };
  }

  OpenCLTuningRecovery::State recoveryState(dir, recoveryProf, tuneFileName, "", recoveryLogFn);
  ...
  OpenCLTuningRecovery::recoveryRemoveFile(autotuneMarkerPath);
